...
public class JogoSingleton extends JPanel implements ActionListener {
	...
	private static JogoSingleton singleton;
	
	public static JogoSingleton getJogo() {
		if(Objects.isNull(singleton)) {
			singleton = new JogoSingleton();
		}
		return singleton;
	}
	...
	private void initJogo() {
		...
		nave = (Nave) NaveFactory.getFactory().createGameElement(40, 60, B_WIDTH);
	...
	private void updateInimigo() {
		while (inimigos.size() < 5) {
			inimigos.add( (Inimigo) InimigoFactory.getFactory().createGameElement(B_WIDTH, random.nextInt(B_HEIGHT - 20) + 10, 0));
		}
